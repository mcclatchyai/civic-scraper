...
def _create_truststore_ssl_context():
    """
    Create an SSL context using truststore library for system certificate store.
    This helps resolve SSL certificate verification errors on sites with
    incomplete certificate chains.
    
    Returns a truststore.SSLContext if available, None otherwise.
    
    Note: We use truststore.SSLContext directly instead of inject_into_ssl()
    because this is a library, not an application. inject_into_ssl() should
    only be used in applications/scripts to avoid conflicts with other libraries.
    """
    try:
        import truststore
        ctx = truststore.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
        logging.getLogger(__name__).debug(
            "Created truststore SSLContext for system certificate store"
        )
        return ctx
    except ImportError:
        logging.getLogger(__name__).debug(
            "truststore not installed. For better SSL compatibility, install with: "
            "pip install truststore (requires Python 3.10+)"
        )
        return None
    except Exception as e:
        logging.getLogger(__name__).debug(f"Failed to create truststore SSLContext: {e}")
        return None


class TruststoreAdapter(HTTPAdapter):
    """
    A requests HTTPAdapter that uses truststore's SSLContext for 
    system certificate store support.
    """
...
class EscribeSite(BaseSite):
    ...
    def __init__(self, url: str, *, place: Optional[str] = None,
        ...
        # - None (default): try truststore for system certs, fallback to default
        # - True: always verify (may fail on sites with cert issues)
        # - False: disable verification (NOT RECOMMENDED - security risk)
        self._verify_ssl = verify_ssl
        if verify_ssl is False:
            warnings.warn(
                "SSL verification is disabled. This is a security risk and should "
                "only be used for debugging. Consider installing truststore instead.",
                UserWarning,
                stacklevel=2
            )
        ...
        # Configure SSL: try truststore SSLContext for system certificate store support
        # This helps with sites that have incomplete certificate chains
        # Note: We use truststore.SSLContext directly (not inject_into_ssl) because this civic-scraper is a library, and inject_into_ssl should only be used in applications
        if self._verify_ssl is not False:
            ssl_context = _create_truststore_ssl_context()
            if ssl_context is not None:
                # Mount the truststore adapter for HTTPS connections
                adapter = TruststoreAdapter(ssl_context=ssl_context)
                self.session.mount('https://', adapter)
                logger.debug("Using truststore SSLContext for HTTPS requests")
        ...
        # Configure session SSL verification
        if self._verify_ssl is False:
            self.session.verify = False
            # Suppress InsecureRequestWarning if verification is explicitly disabled
            import urllib3
